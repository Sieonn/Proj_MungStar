...
public class FreeServiceImpl implements FreeService{
	...
	@Override
	public List<fBoard> searchFreeListByPage(HttpServletRequest request, String query) throws Exception {
		 //1. 페이지를 가져오고 없으면 페이지번호를 1로 한다.
	    String paramPage = request.getParameter("page");
	    Integer page = 1;
	    if (paramPage != null) {
	    ...
	    //2. PageInfo 계산하여 설정하기
	    int boardCount = freeDAO.selectBoardCountByQuery(query);
	    int maxPage = (int) Math.ceil((double) boardCount / 10);
	    int startPage = (page - 1) / 10 * 10 + 1; //페이지 10을 고려하여 page-1을 한다.
	    int endPage = startPage + 10 - 1;
	    if (endPage > maxPage) endPage = maxPage;
	    ...
	    //3. 해당 페이지에 해당하는 검색 결과 목록 조회
	    int row = (page - 1) * 10;
	    List<fBoard> searchResult = freeDAO.searchBoardListByQuery(query, row);

	    return searchResult;
	}
	@Override
	public List<fBoard> search(String query) throws Exception {
		// DAO 메서드를 호출하여 검색 결과를 가져옵니다.
	    return freeDAO.searchBoardListByQuery(query, 0);
	}
}
